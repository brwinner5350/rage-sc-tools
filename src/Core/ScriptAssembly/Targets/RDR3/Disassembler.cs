...
public class Disassembler
{
    ...
    private void DisassembleInstruction(TextWriter w, InstructionContext ctx, uint ip, ReadOnlySpan<byte> inst)
    {
        ...
        switch (opcode)
        {
            ...
            case OpcodeV16.TEXT_LABEL_ASSIGN_STRING:
            case OpcodeV16.TEXT_LABEL_ASSIGN_INT:
            case OpcodeV16.TEXT_LABEL_APPEND_STRING:
            case OpcodeV16.TEXT_LABEL_APPEND_INT:
                w.Write(inst[0]);
                inst = inst[1..];
                break;
            ...
            case OpcodeV16.PUSH_CONST_U24:
            {
                ...
                if (!(opcode is OpcodeV16.CALL && TryWriteFuncLabel(this, w, (uint)value)) &&
                    !(opcode is OpcodeV16.PUSH_CONST_U24 && (TryWriteStringLabel(this, w, ctx, (uint)value) || (hasIndirectCalls && TryWriteFuncLabel(this, w, (uint)value)))))
                {
                    w.Write(value);
                }
                inst = inst[3..];
            ...
            {
                var value = opcode - OpcodeV16.PUSH_CONST_0;
        ...
        {
            var next = ctx.Next();
            if (next.IsValid && next.Opcode is OpcodeV16.STRING && self.stringIndicesById.TryGetValue(strId, out strIndex))
            {
                return true;
            }
            strIndex = default;
            ...
            IterateCode(inst =>
            {
                switch (inst.Opcode)
                {
                    ...
                    case OpcodeV16.ENTER:
                        ...
                        break;
                    case OpcodeV16.LEAVE:
                        addressAfterLastLeaveInst = (uint)(inst.Address + OpcodeV16.LEAVE.ConstantByteSize());
                        break;
                    case OpcodeV16.CALLINDIRECT:
    ...
    private void IdentifyStaticsLabels()
    {
        ...
        if (code.Length != 0)
        {
            IterateCode(inst =>
            {
                ...
                {
                    OpcodeV16.STATIC_U8 or
                    OpcodeV16.STATIC_U8_LOAD or
                    OpcodeV16.STATIC_U8_STORE => inst.Bytes[1],

                    OpcodeV16.STATIC_U16 or
                    OpcodeV16.STATIC_U16_LOAD or
                    OpcodeV16.STATIC_U16_STORE => MemoryMarshal.Read<ushort>(inst.Bytes[1..]),
    ...
    private void IterateCode(IterateCodeCallback callback)
    {
        InstructionContext.CB previousCB = currInst =>
        {
            ...
            {
                prevAddress = address;
                address += (uint)OpcodeTraitsV16.ByteSize(code[(int)address..]);
            }
        ...
        static InstructionContext GetInstructionContext(byte[] code, uint address, InstructionContext.CB previousCB, InstructionContext.CB nextCB)
            ...
            {
                Address = address,
                Bytes = OpcodeTraitsV16.GetInstructionSpan(code, (int)address),
    ...
    private readonly ref struct InstructionContext
    {
        ...
        public OpcodeV16 Opcode => (OpcodeV16)Bytes[0];
