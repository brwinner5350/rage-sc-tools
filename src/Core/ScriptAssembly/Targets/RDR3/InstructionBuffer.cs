...
internal class InstructionBuffer : InstructionBuffer<OpcodeV16, LabelInfo, ScriptPageTable<byte>>
{
    public override ScriptPageTable<byte> Finish(IEnumerable<LabelInfo> labels)
    {
        var segment = new SegmentBuilderRDR3(sizeof(byte), isPaged: true);
        ...
        foreach (var (instOffset, instLength) in InstructionsInfo)
        {
            ...
            OpcodeV16 opcode = (OpcodeV16)instructionBuffer[0];
