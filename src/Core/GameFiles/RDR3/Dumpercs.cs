...
internal class Dumper<TOpcode, TOpcodeTraits> : IDumper
    ...
    where TOpcodeTraits : IOpcodeTraitsRDR3<TOpcode>
{
    public void Dump(Script sc, TextWriter w, DumpOptions options)
    {
        ...
        if (options.IncludeIR)
        {
            ...
            var ir = ScTools.Decompiler.Script.FromRDR3(sc);
    ...
    private static void DisassembleInstructionAt(StringBuilder sb, Script sc, uint ip)
    {
        ...
        ip++;
        foreach (char f in TOpcodeTraits.DumpOpcodeFormats[inst.AsEnum<byte, OpcodeV16>()])
        {
            switch (f)
            {
                case '$':
...
internal class DumperV16 : Dumper<OpcodeV16, OpcodeTraitsV16> { }
